...
<div x-data="{ open: false }" @open-add-user-modal.window="open = true">
    <x-backend::ui.modal :isOpen="false" x-bind:is-open="open" class="max-w-2xl">
        <div class="p-6 sm:p-8">
            <h3 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Add New User</h3>
                ...
                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                    <select name="role" required
                        ...
                        @foreach($roles ?? [] as $role)
                            <option value="{{ $role->name }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">{{ ucfirst($role->name) }}</option>
                        @endforeach
                    </select>
...
">
    <x-backend::ui.modal :isOpen="false" x-bind:is-open="open" class="max-w-2xl">
        <div class="p-6 sm:p-8">
            <h3 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Edit User</h3>

        <form :action="`{{ url('users') }}/${userId}`" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" required x-model="userName"
                    class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                    placeholder="Enter full name" />
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" required x-model="userEmail"
                    class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                    placeholder="user@example.com" />
            </div>
...
<div x-data="{
    ...
    userName = $event.detail.name;
">
    <x-backend::ui.modal :isOpen="false" x-bind:is-open="open" class="max-w-md">
        <div class="p-6 sm:p-8">
        ...
        <h3 class="mb-2 text-xl font-bold text-center text-gray-900 dark:text-white">Delete User</h3>
        ...
        <form :action="`{{ url('users') }}/${userId}`" method="POST" class="space-y-3">
            @csrf
            @method('DELETE')

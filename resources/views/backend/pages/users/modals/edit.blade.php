...
<x-backend::ui.modal
    ...
    <div class="p-6 sm:p-8">
        ...
        {{-- Form: Update User - Action URL is dynamically set based on userId --}}
        <form :action="`{{ url('users') }}/${userId}`" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            {{-- Full Name Field - Bound to userName via x-model --}}
            {{-- Full Name Field - Bound to userName via x-model --}}
            <div>
            ...
            {{-- Email Address Field - Bound to userEmail via x-model --}}
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Email Address
                </label>
            ...
            {{-- Role Selection Field - Bound to userRole via x-model --}}
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                    Role
                </label>
                {{-- Alpine.js scope for select styling (starts as selected since editing existing user) --}}
